...
class TestUserManagement(TestCase):
    def setUp(self):
        ...
        self.superuser = CustomUser.objects.create_superuser('django2',
                                                             'django2@geekshop.local',
                                                             'geekbrains')

        self.user = CustomUser.objects.create_user('tarantino',
                                                   'tarantino@geekshop.local',
                                                   'geekbrains')

        self.user_with__first_name = CustomUser.objects.create_user(
            'umaturman',
    ...
    def test_basket_login_redirect(self):
        ...
        self.assertIn('Cart', response.content.decode())
    ...
    def test_user_register(self):
        ...
        new_user = CustomUser.objects.get(username=new_user_data['username'])
    ...
    def test_user_wrong_register(self):
        ...
        response = self.client.post('/auth/register/', data=new_user_data)
        self.assertEqual(response.status_code, 200)
        self.assertFormError(response, 'registration_form', 'age', 'Регистрация доступна для пользователей старше 18 лет!')
